...
These immutable copies are the old-model side of the future source-disjoint benchmark comparison. Fine-tuning outputs must use new candidate filenames and must not overwrite either snapshot.
...
`models/best.pt` now contains this checkpoint. The previous detector remains available as `detector_baseline_before_dataset3.pt`; exact deployment and rollback settings are recorded in `models/deployment.json`.
