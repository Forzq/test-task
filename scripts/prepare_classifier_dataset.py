...
class PreparationConfig:
    ...
    context_ratio : float
        Context added around YOLO boxes and proposals.
...
def parse_config() -> PreparationConfig:
    ...
    config = PreparationConfig(
        ...
        context_ratio=arguments.context,
...
def prepare_dataset(config: PreparationConfig) -> dict[str, dict[str, int]]:
    ...
    _write_manifest(config, positives, negatives, selected_counts)
    return {
        split: {
            class_name: sum(
                count
